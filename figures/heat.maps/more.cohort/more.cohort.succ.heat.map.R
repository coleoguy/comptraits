...
#Read in more.succ.matrix.csv
dat <- read.csv("more.cohort.succ.matrix.csv")
...
#remove "total" column
more.cohort.succ.matrix <- dat[,1:7]

# Get row and column names
row_names <- rownames(more.cohort.succ.matrix)
col_names <- colnames(more.cohort.succ.matrix)


# Melt the table to long format
library(reshape2)

more.cohort.succ.matrix.long <- melt(more.cohort.succ.matrix)


# Create the plot
ggplot(more.cohort.succ.matrix.long, aes(x = variable, y = Elemental.Architectures, fill = value)) +
  geom_tile() +

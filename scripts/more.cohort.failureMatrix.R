...
colnames(more.cohort.fail.matrix) <- c("first element", "second element", "1/2", "2/1", "1*2", "1+2", "1-2", "2-1", "total")
...
for(i in 1:(length(starts)-1)){
  ...
  for(j in 3:8){
    ...
    x <- more.cohort.fail.matrix$`first element` == foo[1,1]
    y <- more.cohort.fail.matrix$`second element` == foo[2,1]
    targ.row <- which(x+y == 2)
    if(a | b){
      print("success")
      # get the row to enter into
      more.cohort.fail.matrix[targ.row, j] <- more.cohort.fail.matrix[targ.row, j] + 1
      more.cohort.fail.matrix[targ.row, 9] <- more.cohort.fail.matrix[targ.row, 9] +1
    }else{
      print("failed")
      more.cohort.fail.matrix[targ.row, 9] <- more.cohort.fail.matrix[targ.row, 9] +1
    }
...
more.cohort.fail.matrix[,3:8] <- round(more.cohort.fail.matrix[,3:8] / (more.cohort.fail.matrix$total/6), digits=2)
write.csv(more.cohort.fail.matrix, "../results/more.cohort.fail.matrix.csv", row.names = F)

# line cross analysis simulations for a larger cohort set 
# adding all crosses from J. Demuth datasets (15 total cohorts)
library(SAGA2)
...
# For this sim, we will use additional cohorts: 
# P1, P2, F1, rF1, F2a, F2b, rF2b, BC1a, rBC1a, BC1b, rBC1b, BC2a, rBC2a, BC2b, rBC2b
...
blank <- read.csv("../data/jd.cross.structure.csv")

# get the generating c-matrix for the increased number of cohorts
cmat <- read.csv("../data/sim data/jd.gen.cmat.csv")[,-1]
replicate <- 1000
...
#write.csv(results,file="../results/jd.sim.results.csv", row.names = F)

...
more.cohort.succ.matrix <- as.data.frame(matrix(0, nrow = 18, ncol = 9))
more.cohort.succ.matrix[, 1] <- c("a", "a", "d", "a", "a", "a", "d", "d", "d", "aa", "aa", "aa", "ad", "ad", "ad", "dd", "dd", "dd")
more.cohort.succ.matrix[, 2] <- c("a", "d", "d", "aa", "ad", "dd", "aa", "ad", "dd", "aa", "ad", "dd", "aa", "ad", "dd", "aa", "ad", "dd")
colnames(more.cohort.succ.matrix) <- c("first element", "second element", "1/2", "2/1", "1*2", "1+2", "1-2", "2-1", "total")
...
for(i in 1:(length(starts)-1)){
  ...
  for(j in 3:8){
    ...
    x <- more.cohort.succ.matrix$`first element` == foo[1,1]
    y <- more.cohort.succ.matrix$`second element` == foo[2,1]
    targ.row <- which(x+y == 2)
    if(all(c(a,b,c))){
      print("success")
      # get the row to enter into
      more.cohort.succ.matrix[targ.row, j] <- more.cohort.succ.matrix[targ.row, j] + 1
      more.cohort.succ.matrix[targ.row, 9] <- more.cohort.succ.matrix[targ.row, 9] +1
    }else{
      print("failed")
      more.cohort.succ.matrix[targ.row, 9] <- more.cohort.succ.matrix[targ.row, 9] +1
    }
...
more.cohort.succ.matrix[,3:8] <- round(more.cohort.succ.matrix[,3:8] / (more.cohort.succ.matrix$total/6), digits=2)
write.csv(more.cohort.succ.matrix, "../results/more.cohort.succ.matrix.csv", row.names = F)

...
jd.part.matrix <- as.data.frame(matrix(0, nrow = 18, ncol = 9))
jd.part.matrix[, 1] <- c("a", "a", "d", "a", "a", "a", "d", "d", "d", "aa", "aa", "aa", "ad", "ad", "ad", "dd", "dd", "dd")
jd.part.matrix[, 2] <- c("a", "d", "d", "aa", "ad", "dd", "aa", "ad", "dd", "aa", "ad", "dd", "aa", "ad", "dd", "aa", "ad", "dd")
colnames(jd.part.matrix) <- c("first element", "second element", "1/2", "2/1", "1*2", "1+2", "1-2", "2-1", "total")
...
for(i in 1:(length(starts)-1)){
  ...
  for(j in 3:8){
    ...
    x <- jd.part.matrix$`first element` == foo[1,1]
    y <- jd.part.matrix$`second element` == foo[2,1]
    targ.row <- which(x+y == 2)
    if(all(c(a,b))){
      print("success")
      # get the row to enter into
      jd.part.matrix[targ.row, j] <- jd.part.matrix[targ.row, j] + 1
      jd.part.matrix[targ.row, 9] <- jd.part.matrix[targ.row, 9] +1
    }else{
      print("failed")
      jd.part.matrix[targ.row, 9] <- jd.part.matrix[targ.row, 9] +1
    }
...
jd.part.matrix[,3:8] <- round(jd.part.matrix[,3:8] / (jd.part.matrix$total/6), digits=2)
write.csv(jd.part.matrix, "../../../results/additional.cohorts.results/jd.cohorts15/jd.part.matrix.csv", row.names = F)
